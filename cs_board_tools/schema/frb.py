"""Definition and boilerplate functions for Fortune Avenue-compatible .frb
files.
"""
...
class SquareType(Enum):
    """The SquareType enum represents the various types of Squares that can
    exist in a Fortune Avenue-compatible .frb file.

    This enum allows us to refer to them by name, rather than referring to
    their bytecodes all over the place.
    """
    Property: U16 = 0x00
    Bank: U16 = 0x01
...
class LoopingMode(Enum):
    """The LoopingMode enum works the same way, allowing
    us to refer to the bytecodes that represent None,
    Vertical, and Both (Horizontal and Vertical) values of
    LoopingMode.
    """
    NONE: U16 = 0
    VERTICAL: U16 = 2
...
class MagicNumber(Enum):
    """In this case, the Magic Number represents specific types
    of data, each of which is a separate subset of the .frb file.
    We use an enum here to make these easier to reference.
    """
...
class Header:
    ...
    This is also part of the .frb format.
    """
...
class WaypointData:
    ...
    This is pathing data -- there is an entry ID, then a list of up to three
    destinations for that entry ID. This determines which Square IDs the
    player is allowed to visit based on where they've come from (the Entry
    ID is a Square ID).
    """
    entryId: U8
...
class Square:
    ...
    This object contains a square type, position information, waypoints,
    district ID, one way lift boolean, a value, a price, and a reference
    to a shop model.
    """
    _square_type: U16
    positionX: I16
...
@dataclass
class BoardData:
    """A dataclass representing Board Data, which is attached
    to a Square object.

    This object contains "parameter" information -- information
    about the specifics of the squares in the board. The object
    contains a header, two unknown values, a square count, and a
    list of squares.
    """
    ...
    # we have to put 0 as length for the list of squares,
    # since bytechomp does not support it properly yet
...
@dataclass
class BoardInfo:
    """A dataclass representing Board Info, which is attached
    to a Square object.

    This object contains global board information. The object
    contains a header, an unknown value, initial cash, target
    amount, base salary, salary increment, max dice roll, looping
    mode (galaxy status here), and a version flag, which is added
    by newer versions of Fortune Avenue.
    """
    _header: Header = field(repr=False, compare=False)
...
@dataclass
class BoardFile:
    """A dataclass representing Board Info, which is attached to
    a Square object.
    ...
    def normalize(self):
        ...
        self._board_data._header.header_size = (
            BoardData.size() + Square.size() * len(self._board_data.squares)
        )


def read_frb(file_path: Path) -> BoardFile:
    """
    Reads an .frb file in and returns a BoardFile object
    representing it.

    :param file_path: A filename for a Fortune Avenue-compatible
        .frb board file.
    :type file_path: str
    :return: Returns a BoardFile object, containing all of the
        values that live inside the .frb file we provided.
    :rtype: BoardFile
    """
        ...
        while data := fp.read(1):
            ...
            if reader.is_complete():
                if board_file is None:
                    ...
                    # now that we know the size of the squares,
                    # read the rest of it
                    @dataclass
                    class Squares:
                        squares: Annotated[
                    ...
                    # we read the squares and put it into our original
                    # board_file struct
                    board_file._board_data.squares = reader.build().squares
                    # and now read the rest of the custom meta data
                    # (i.e. advanced auto-path data, auto-path range, etc.)
                    bytes_left = fp.read()
...
def write_frb(board_file: BoardFile, file_path: Path):
    """
    Writes a Fortune Avenue-compatible .frb file from a
    user-provided BoardFile object.
